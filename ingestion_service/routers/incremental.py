...
from ingestion_service.core.incremental_updater import ingest_merged_pr, ingest_direct_push
...
async def handle_pr_merge(payload: IncrementalPRPayload):
    ...
    logger.info("PR merge ingestion %s: %s/%s#%d", job_id, payload.owner, payload.repo_name, payload.pr_number)
    ...
    try:
        ...
        logger.info(
            ...
            job_id, stats.files_added, stats.files_modified, stats.files_deleted, len(stats.errors),
        )
