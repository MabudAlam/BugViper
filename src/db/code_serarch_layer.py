...
class CodeSearchService:
    ...
    def peek_file_lines(
        self, path: str, line: int = 1, above: int = 10, below: int = 10, repo_id: str | None = None
    ) -> Dict[str, Any]:
        """Return a window of lines around a specific line in a file.

        For markdown files, returns the full content.
        """
        if repo_id:
            records, _, _ = self.db.run_query(
                """
        ...
        is_markdown = records[0].get("is_markdown", False)
        ...
        if is_markdown:
            return {
                ...
                "total_lines": total,
                "window": [
                    {"line_number": i + 1, "content": lines[i], "is_anchor": True}
    ...
    def get_readme(self, repo_id: str) -> Optional[Dict[str, Any]]:
        """Fetch the README file for a repository if it exists."""
        records, _, _ = self.db.run_query(
            """
            MATCH (f:File)
    ...
    def peek_readme_lines(
        self, repo_id: str, path: str, line: int = 1, above: int = 50, below: int = 50
    ) -> Dict[str, Any]:
        """Return a window of lines around a specific line in a README file, filtered by repo."""
        records, _, _ = self.db.run_query(
            """
            MATCH (f:File)
            WHERE f.repo = $repo_id AND f.path = $path AND f.source_code IS NOT NULL
        ...
        if not records or not records[0].get("source_code"):
            return {"error": "README not found", "path": path}
