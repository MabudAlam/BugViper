...
def get_tools(query_service: CodeSearchService, repo_id: str | None = None) -> list:
    ...
    @tool(response_format="content_and_artifact")
    def peek_code(path: str, line: int, above: int = 100, below: int = 50) -> tuple[str, list[_S]]:
        ...
        Args:
            ...
            above: Lines to show above the anchor (default 100, max 200).
            below: Lines to show below the anchor (default 50, max 200).
    ...
    @tool(response_format="content_and_artifact")
    def get_readme() -> tuple[str, list[_S]]:
        """Read the entire README file for the selected repository.
        Use to understand what the project is about, how to set it up, and its main features.
        Returns: The complete README content.
        """
        if not repo_id:
            return "No repository selected. Please select a repository first.", []
        result = query_service.get_readme(repo_id)
        if not result:
            return f"No README found for repository '{repo_id}'.", []
        source = result.get("source_code") or ""
        path = result.get("path")
        header = f"README: {path}"
        return f"{header}\n{'=' * 60}\n{source}", [_src(path, typ="readme")]

    # ── 21. Find README path ────────────────────────────────────────────────

    @tool(response_format="content_and_artifact")
    def find_readme_path() -> tuple[str, list[_S]]:
        """Find the file path of the README for the selected repository.
        Returns: README file path and name.
        """
        if not repo_id:
            return "No repository selected. Please select a repository first.", []
        result = query_service.get_readme(repo_id)
