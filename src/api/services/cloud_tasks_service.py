...
class CloudTasksService:
    ...
    def is_enabled(self) -> bool:
        # Cloud Tasks dispatch requires both a target service URL and a GCP project.
        return bool(self._service_url and self._project)

    @property
    def review_is_enabled(self) -> bool:
        return bool(self._review_url and self._project)
    ...
    def _dispatch(
        ...
        if not self._project:
            logger.warning("Cloud Tasks not enabled — GCP_PROJECT_ID is unset")
            return None
        ...
        try:
            ...
            logger.exception("Failed to initialize Cloud Tasks client")
            return None
        ...
        try:
            ...
            logger.exception("Failed to dispatch Cloud Task to %s", url)
            return None
