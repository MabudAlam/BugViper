...
from knowledge_parser.tree_sitter_manager import execute_query
...
class CSharpLangTreeSitterParser:
            ...
            for capture_name, query_str in CSHARP_QUERIES.items():
                ...
                if capture_name == "functions":
                    parsed_functions = self._parse_functions(
                        captures, source_code, path, tree.root_node
                    )
                elif capture_name == "classes":
                    parsed_classes = self._parse_type_declarations(
                        captures, source_code, path, "Class"
                    )
                elif capture_name == "interfaces":
                    parsed_interfaces = self._parse_type_declarations(
                        captures, source_code, path, "Interface"
                    )
                elif capture_name == "structs":
                    parsed_structs = self._parse_type_declarations(
                        captures, source_code, path, "Struct"
                    )
                elif capture_name == "enums":
                    parsed_enums = self._parse_type_declarations(
                        captures, source_code, path, "Enum"
                    )
                elif capture_name == "records":
                    parsed_records = self._parse_type_declarations(
                        captures, source_code, path, "Record"
                    )
                elif capture_name == "properties":
                    parsed_properties = self._parse_properties(
                        captures, source_code, path, tree.root_node
                    )
    ...
    def _parse_functions(
        self, captures: list, source_code: str, path: Path, root_node
    ) -> list[Dict[str, Any]]:
        functions = []
    ...
    def _parse_type_declarations(
        self, captures: list, source_code: str, path: Path, type_label: str
    ) -> list[Dict[str, Any]]:
