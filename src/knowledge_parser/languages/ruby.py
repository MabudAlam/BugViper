...
class RubyLangTreeSitterParser:
    ...
    def _get_parent_context(
        self, node: Any, types: Tuple[str, ...] = ("class", "module", "method")
    ):
    ...
    def _find_calls(self, root_node: Any) -> list[Dict[str, Any]]:
        ...
        for node, capture_name in all_captures:
            for call_id, call_data in captures_by_call.items():
                ...
                if not (
                    node.start_byte >= call_node.start_byte and node.end_byte <= call_node.end_byte
                ):
                    continue
                ...
                if capture_name == "name":
                    ...
                    captures_by_call[call_id]["args"] = [
                        a.strip() for a in args_text.strip("()").split(",") if a.strip()
                    ]
        ...
        for call_data in captures_by_call.values():
            ...
            if name:
                ...
                if context_type == "method":
                    ...
                    enclosing_class, _, _ = self._get_parent_context(
                        call_node.parent, ("class", "module")
                    )
