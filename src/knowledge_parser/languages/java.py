...
from common import debug_log, error_logger, warning_logger
                ...
                try:
                    ...
                    if name_node:
                        ...
                        if interfaces_node:
                            ...
                            if not type_list:
                                type_list = next(
                                    (c for c in interfaces_node.children if c.type == "type_list"),
                                    None,
                                )
    ...
    for path in files:
        try:
            ...
            interface_matches = re.finditer(
                r"\b(?:public\s+|private\s+|protected\s+)?interface\s+(\w+)", content
            )
...
def _find_annotations(tree):
    ...
    for node in tree.find_all("annotation_type_declaration"):
        ...
        annotations.append({"type": "Annotation", "name": name, "location": node.start_point})
    return annotations
...
def _find_applied_annotations(tree):
    ...
    for node in tree.find_all("marker_annotation"):
        name = node.child_by_field_name("name").text
        applied.append({"type": "AnnotationUsage", "name": name, "location": node.start_point})
    return applied
