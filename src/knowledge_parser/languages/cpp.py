...
class CppLangTreeSitterParser:
    ...
    def parse(
        self, path: Path, is_dependency: bool = False, index_source: bool = False, **kwargs
    ) -> Dict:
            ...
            if capture_name == "function_name":
                ...
                for n, cap in execute_query(self.language, query_str, func_node):
                    if cap == "return_type":
                        return_type_node = n
                        break
                self._get_node_text(return_type_node) if return_type_node else None
