...
import { Bot, User, Send, Loader2, ChevronDown, FileCode, ChevronRight, X, SquarePen, Trash2 } from "lucide-react";
...
function SourceSheet({ source, onClose }: { source: AgentSource | null; onClose: () => void }) {
  ...
  const [above, setAbove] = useState(30);
  const [below, setBelow] = useState(30);

  const load = useCallback((a: number, b: number) => {
    if (!source) return;
    let cancelled = false;
    peekFileLines(source.path, source.line_number ?? 1, a, b)
      .then((res) => {
  ...
  return createPortal(
    <>
      ...
      <div className="fixed right-0 top-0 z-50 h-full w-full max-w-[560px] flex flex-col bg-background border-l border-border shadow-2xl">
        ...
        {/* Navigation buttons */}
        {peekData && (
          <div className="flex items-center gap-2 px-5 py-2 border-b border-border text-xs text-muted-foreground">
            <button
              className="hover:text-foreground disabled:opacity-40"
              disabled={isLoading}
              onClick={() => { setAbove(a => a + 20); }}
            >
              ↑ 20 above
            </button>
            <span>·</span>
            <button
              className="hover:text-foreground disabled:opacity-40"
              disabled={isLoading}
              onClick={() => { setBelow(b => b + 20); }}
            >
              20 below ↓
            </button>
            <span className="ml-auto">
              lines {peekData.window[0]?.line_number}–{peekData.window[peekData.window.length - 1]?.line_number} / {peekData.total_lines}
            </span>
          </div>
        )}
...
export default function AgentPage() {
  ...
  const handleNewChat = async () => {
    await clearMySession(selectedRepo).catch(() => {});
    setMessages([]);
  };
  ...
  const send = async () => {
    ...
    if (!question || loading || !selectedRepo) return;
    ...
    <>
      ...
      */}
      <div className="-m-8 h-screen flex flex-col overflow-hidden">
        ...
        {showDeleteDialog && selectedRepo && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-background border border-border rounded-xl shadow-xl p-6 w-full max-w-sm mx-4">
              <h3 className="text-sm font-semibold mb-2">Delete chat history?</h3>
              <p className="text-xs text-muted-foreground mb-4">
                This will permanently delete all messages for <strong>{selectedRepo}</strong>. This action cannot be undone.
              </p>
