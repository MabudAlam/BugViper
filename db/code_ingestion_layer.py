...
def _serialize_params(params: Optional[List[Dict[str, Any]]]) -> str:
    ...
    if not params:
        return "[]"
    cleaned = [{"name": p.get("name", ""), "type": p.get("type") or ""} for p in params]
...
class GraphIngestionService:
    ...
    def create_symbol(
        ...
        self.db.run_query(link_query, {"symbol_id": symbol_id, "definition_id": definition_id})
    ...
    def create_user(self, username: str, email: Optional[str] = None) -> Dict[str, Any]:
        ...
        records, _, _ = self.db.run_query(query, {"username": username, "email": email})
        ...
        if base_classes:
            for base in base_classes:
                self._inheritance_relationships.append(
                    {"child_id": class_id, "parent_name": base, "repo_id": file_id.split(":")[0]}
                )
    ...
    def batch_create_classes(self, file_id: str, classes_data: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        records, _, _ = self.db.run_query(query, {"file_id": file_id, "classes": classes_data})

        self.stats.classes_created += len(classes_data)
        return [r["class_id"] for r in records] if records else []

    def batch_create_methods(self, class_id: str, methods_data: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        records, _, _ = self.db.run_query(query, {"class_id": class_id, "methods": methods_data})

        self.stats.methods_created += len(methods_data)
        return [r["method_id"] for r in records] if records else []

    def batch_create_functions(
        self, file_id: str, functions_data: List[Dict[str, Any]]
    ) -> List[str]:
        """
        ...
        records, _, _ = self.db.run_query(query, {"file_id": file_id, "functions": functions_data})

        self.stats.functions_created += len(functions_data)
        return [r["function_id"] for r in records] if records else []

    def batch_create_imports(self, file_id: str, imports_data: List[Dict[str, Any]]) -> None:
        """Create multiple import nodes in a single query."""
        if not imports_data:
            return

        query = CYPHER_QUERIES["batch_create_imports"]
        self.db.run_query(query, {"file_id": file_id, "imports": imports_data})

        self.stats.imports_created += len(imports_data)

    def batch_create_variables(self, file_id: str, variables_data: List[Dict[str, Any]]) -> None:
        """Create multiple variable nodes in a single query."""
        if not variables_data:
            return

        query = CYPHER_QUERIES["batch_create_variables"]
        self.db.run_query(query, {"file_id": file_id, "variables": variables_data})
    ...
    def process_deferred_relationships_batch(self) -> None:
        ...
        if self._call_relationships:
            try:
                ...
                self.db.run_query(
                    query,
                    {"calls": self._call_relationships, "repo_id": self._current_repo_id or ""},
                )
        ...
        if self._inheritance_relationships:
            try:
                ...
                print(
                    f"  ✓ Processed {len(self._inheritance_relationships)} inheritance relationships"
                )
            except Exception as e:
                print(f"  ✗ Batch inheritance failed: {e}")

        self.stats.relationships_created = len(self._call_relationships) + len(
            self._inheritance_relationships
        )
    ...
    def process_deferred_relationships(self) -> None:
        ...
        for i, rel in enumerate(self._call_relationships):
            symbol_id = self._find_symbol_by_name(rel["callee_name"], self._current_repo_id)
        ...
        for i, rel in enumerate(self._inheritance_relationships):
            ...
            try:
                ...
                if failed_inheritance <= 3:
                    print(
                        f"  ✗ Failed to create INHERITS: {rel.get('child_id')} -> {rel.get('parent_name')}"
                    )
                    print(f"    Error: {e}")
        ...
        success_rate = (
            (total_created / (total_created + total_failed) * 100)
        ...
        for cls in file_analysis.classes:
            ...
            if cls.base_classes:
                for base in cls.base_classes:
                    self._inheritance_relationships.append(
                        {"child_id": class_id, "parent_name": base, "repo_id": repo_id}
                    )
            ...
            if cls.methods:
                ...
                for method in cls.methods:
                    ...
                    method_qualified_name = self._build_qualified_name(
                        method.name, scope="method", class_name=cls.name
                    )
                    ...
                    methods_data.append(
                        {
                            ...
                            "decorators": [d.name for d in method.decorators]
                        ...
                        self._call_relationships.append(
                            {
                                ...
                                if call.range and call.range.start
                                else 0,
                            }
                        )
                ...
                functions_data.append(
                    {
                        ...
                        "params": _serialize_params(
                            [{"name": p.name, "type": p.type_annotation} for p in func.parameters]
                        ),
    ...
    def create_script(self, config_id: str, name: str, command: str) -> str:
        """
        ...
        records, _, _ = self.db.run_query(
            query,
            {"config_id": config_id, "script_id": script_id, "name": name, "command": command},
        )
    ...
    def ingest_config_file(self, config_analysis: Any, repo_id: str) -> str:
        ...
        for script in config_analysis.scripts:
            self.create_script(config_id=config_id, name=script.name, command=script.command)
